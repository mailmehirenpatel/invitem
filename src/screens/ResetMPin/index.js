...
import {createMPinApi, requestResetMpin} from '../../store/actions/AuthAction';
...
const ResetMPin = ({navigation, route}) => {
  ...
  const createMPin = useCallback(
            ...
            requestResetMpin(
              ...
              (isSuccess, message) => {
                if (isSuccess) {
                  ...
                  navigation.replace(NavigationRoutes.MpinCode);
    ...
    [EnteredPin, dispatch, email, navigation],
  );
    ...
    <View style={styles.confirmPinMainContainer}>
      ...
      <View style={styles.confirmPinTopContainer}>
        ...
        <Text style={styles.confirmPinMainHeading}>
          <Text>
            {EnteredPin.length !== 4
              ? Strings.enterYourNewMPin
              : Strings.ConfirmYourNewMPin}
          </Text>
        </Text>
      </View>
